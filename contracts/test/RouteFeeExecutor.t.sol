...
contract FeeReentry {
    ...
    constructor(RouteFeeExecutor value) {
        executor = value;
    }
...
contract RouteFeeExecutorTest {
    ...
    function testDirectFeeAndExactApprovals() public {
        ...
        require(
            a.allowance(address(fees), address(route)) == 0
                && a.allowance(address(route), address(adapter)) == 0
        );
    ...
    function testMetaFeeAndExactApprovals() public {
        ...
        require(
            a.allowance(address(fees), address(meta)) == 0
                && a.allowance(address(meta), address(upstream)) == 0
        );
    }
    ...
    function testInvalidFeeRecipientsRejected() public {
        ...
        for (uint256 i; i < invalid.length; ++i) {
            vm.expectRevert(RouteFeeExecutor.InvalidFeeRecipient.selector);
    ...
    function testAdminRenounceDisabled() public {
        vm.expectRevert(RouteFeeExecutor.RenounceDisabled.selector);
    ...
    function testRejectingTreasuryRevertsAll() public {
        ...
        uint256 beforeInput = a.balanceOf(user);
        vm.prank(user);
        vm.expectRevert(RouteFeeExecutor.NativeTransferFailed.selector);
        fees.swap(
    ...
    function testTreasuryReentryBlocked() public {
        FeeReentry receiver = new FeeReentry(fees);
    ...
    function testTaxedInputRejected() public {
        ...
        vm.prank(user);
        vm.expectRevert(RouteFeeExecutor.UnsupportedToken.selector);
        fees.swap(
    ...
    function testExpiredSwapPaysNoFee() public {
        vm.warp(100);
        vm.prank(user);
        vm.expectRevert(RouteFeeExecutor.DeadlineExpired.selector);
