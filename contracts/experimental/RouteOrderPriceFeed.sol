...
contract RouteOrderPriceFeed is IOrderPriceFeed {
    ...
    constructor(Config[] memory configs, address uptimeFeed, uint256 grace) {
        if (
            configs.length == 0 || configs.length > 32 || uptimeFeed.code.length == 0 || grace == 0
                || grace > 1 days
        ) {
        ...
        for (uint256 i; i < configs.length; ++i) {
            ...
            if (
                c.token.code.length == 0 || c.feed.code.length == 0 || c.maxAge == 0
                    || c.maxAge > 1 hours || address(sources[c.token].feed) != address(0)
            ) {
                revert InvalidConfig();
            }
    ...
    function price(address token) external view returns (uint256 value, uint256 updatedAt) {
        ...
        if (
            status != 0 || since == 0 || since > block.timestamp || reportedAt < since
        ...
        if (
            answer <= 0 || at == 0 || at > block.timestamp || block.timestamp - at > s.maxAge
                || s.feed.decimals() != s.decimals
        ) {
            revert InvalidPrice();
        }
