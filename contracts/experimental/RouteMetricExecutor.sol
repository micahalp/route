...
contract RouteMetricExecutor is ReentrancyGuardTransient {
    ...
    constructor(address poolFactory, address wrapped) {
        if (poolFactory.code.length == 0 || wrapped.code.length == 0) {
            revert InvalidRoute();
        }
        factory = poolFactory;
        wrappedNative = IRouteMetricWrapped(wrapped);
    }
    ...
    function swap(
        ...
        if (amountIn == 0 || amountIn > uint256(uint128(type(int128).max)) || minimum == 0) {
            revert InvalidAmount();
        }
        if (
            recipient == address(0) || recipient == address(this) || recipient == pool
                || pool.code.length == 0 || !IRouteMetricFactory(factory).isPool(pool)
        ) {
            revert InvalidRoute();
        }
        ...
        if (input == output || msg.value != (tokenIn == address(0) ? amountIn : 0)) {
            revert InvalidAmount();
        }
        ...
        if (
            p.factory != factory || p.token0 == p.token1
                || !((input == p.token0 && output == p.token1)
                    || (input == p.token1 && output == p.token0)) || p.extension1 != address(0)
                || p.extension2 != address(0) || p.extension3 != address(0)
                || p.extension4 != address(0) || p.extension5 != address(0)
                || p.extension6 != address(0) || p.extension7 != address(0) || p.beforeAdd != 0
                || p.afterAdd != 0 || p.beforeRemove != 0 || p.afterRemove != 0 || p.beforeSwap != 0
                || p.afterSwap != 0
        ) {
            revert InvalidRoute();
        }
        ...
        if (tokenIn == address(0)) {
            ...
            if (wrappedNative.balanceOf(address(this)) != inputBefore + amountIn) {
                revert UnsupportedToken();
            }
        }
        callbackPool = pool;
        callbackToken = input;
        callbackPayer = tokenIn == address(0) ? address(this) : msg.sender;
        callbackAmount = amountIn;
        callbackDirection = input == p.token0;
        (int128 d0, int128 d1) = IRouteMetricPool(pool)
            .swap(
                outputRecipient,
                callbackDirection,
        ...
        if (
            spent != int256(amountIn) || received >= 0 || uint256(-received) != amountOut
                || callbackOutput != amountOut
        ) {
            revert InvalidAmount();
        }
        ...
        delete callbackPayer;
        delete callbackDirection;
        ...
        if (
            IERC20(input).balanceOf(address(this)) != inputBefore
                || IERC20(output).balanceOf(address(this)) != outputBefore
                || address(this).balance != nativeBefore
                || (tokenIn != address(0)
                    && IERC20(input).balanceOf(msg.sender) + amountIn != senderBefore)
        ) {
            revert UnsupportedToken();
        }
    ...
    function metricOmmSwapCallback(int256 delta0, int256 delta1, bytes calldata data) external {
        if (callbackPool == address(0) || msg.sender != callbackPool || data.length != 0) {
            revert CallbackRejected();
        }
        int256 owed = callbackDirection ? delta0 : delta1;
        int256 received = callbackDirection ? delta1 : delta0;
        if (
            owed <= 0 || uint256(owed) != callbackAmount || received >= 0
                || received < -int256(type(int128).max)
        ) {
            revert CallbackRejected();
        }
        IERC20 token = IERC20(callbackToken);
        address payer = callbackPayer;
        delete callbackPool;
        delete callbackAmount;
        callbackOutput = uint256(-received);
        uint256 beforePool = token.balanceOf(msg.sender);
        if (payer == address(this)) {
            token.safeTransfer(msg.sender, uint256(owed));
        } else {
            token.safeTransferFrom(payer, msg.sender, uint256(owed));
        }
        if (token.balanceOf(msg.sender) != beforePool + uint256(owed)) {
            revert UnsupportedToken();
        }
    }
}
