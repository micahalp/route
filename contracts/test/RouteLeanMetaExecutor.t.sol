...
contract LeanReentrantRecipient {
    ...
    constructor(RouteLeanMetaExecutor target, bool shouldReject) {
        guard = target;
        reject = shouldReject;
    }
    ...
    receive() external payable {
        ...
        try guard.swap(
            address(0), address(1), 1, 1, address(this), block.timestamp, hex"12345678"
        ) {
...
contract RouteLeanMetaExecutorTest {
    ...
    function payload(uint256 amount, uint256 output, address receiver)
        internal
        view
    ...
    function testPartialInputRejected() public {
        vm.expectRevert(RouteLeanMetaExecutor.UnsupportedToken.selector);
        execute(1 ether, 2 ether, recipient, payload(0.5 ether, 2 ether, recipient));
    }
    ...
    function testExpiredRejected() public {
        vm.warp(100);
        vm.prank(user);
        vm.expectRevert(RouteLeanMetaExecutor.DeadlineExpired.selector);
        guard.swap(
            address(a),
            address(b),
            1 ether,
            ...
            recipient,
            99,
            payload(1 ether, 2 ether, recipient)
        );
    }
    ...
    function testNativeInput() public {
        ...
        require(
            guard.swap{value: 1 ether}(
                address(0), address(b), 1 ether, 2 ether, recipient, block.timestamp, data
            ) == 2 ether
        );
    ...
    function testTaxedInputRevertsAtomically() public {
        ...
        taxed.mint(user, 2 ether);
        vm.prank(user);
        taxed.approve(address(guard), 1 ether);
        bytes memory data = abi.encodeCall(
            MockUpstream.swap, (address(taxed), address(b), 1 ether, 2 ether, recipient)
        );
        vm.prank(user);
        vm.expectRevert(RouteLeanMetaExecutor.UnsupportedToken.selector);
