...
contract RouteExecutionHub is ReentrancyGuard, Ownable2Step {
    ...
    constructor(address wrapped, address[] memory adapters, address v4, address upstream, bytes4 selector, address admin, address treasury) Ownable(admin) {
        if (wrapped.code.length == 0 || upstream.code.length == 0 || adapters.length == 0 || adapters.length > 16 || selector == bytes4(0)) revert InvalidRoute();
    ...
    receive() external payable {
        ...
        if (msg.sender != address(wrappedNative) && !acceptingNative) revert InvalidRoute();
    }
    ...
    function _setFeeRecipient(address treasury) private {
        if (treasury == address(0) || treasury == address(this) || treasury == aggregator || treasury == address(wrappedNative) || allowedAdapter[treasury]) revert InvalidFeeRecipient();
    ...
    function grossForNet(uint256 net) public pure returns (uint256) { return net == 0 ? 0 : Math.mulDiv(net - 1, 10_000, 10_000 - FEE_BPS) + 1; }
    function _balance(address token) private view returns (uint256) { return token == address(0) ? address(this).balance : IERC20(token).balanceOf(address(this)); }
    function _check(address tokenIn, address tokenOut, uint256 amountIn, uint256 minOut, address recipient, uint256 deadline) private view {
        ...
        uint256 deadline, RouteExecutor.Branch[] calldata branches) external payable nonReentrant returns (uint256 amountOut)
    {
        ...
        address input = tokenIn == address(0) ? address(wrappedNative) : tokenIn;
        address output = tokenOut == address(0) ? address(wrappedNative) : tokenOut;
        if (branches.length == 0 || branches.length > 3) revert InvalidRoute();
        uint256 sum;
        for (uint256 b; b < branches.length; ++b) {
            RouteExecutor.Branch calldata branch = branches[b];
            if (branch.amountIn == 0 || branch.legs.length == 0 || branch.legs.length > 3) revert InvalidRoute();
            sum += branch.amountIn;
        ...
        if (tokenOut == address(0)) wrappedNative.withdraw(gross);
    ...
    {
        _check(tokenIn, tokenOut, amountIn, minOut, recipient, deadline);
        if (data.length < 4 || data.length > 65536 || bytes4(data[:4]) != swapSelector) revert InvalidRoute();
        address treasury = feeRecipient;
        uint256 beforeInput = _balance(tokenIn) - msg.value;
        uint256 beforeOutput = _balance(tokenOut);
        if (tokenIn != address(0)) {
            IERC20(tokenIn).safeTransferFrom(msg.sender, address(this), amountIn);
            if (_balance(tokenIn) != beforeInput + amountIn) revert UnsupportedToken();
        ...
        if (_balance(tokenIn) != beforeInput) revert UnsupportedToken();
        uint256 gross = _balance(tokenOut) - beforeOutput;
        amountOut = _payOutput(tokenOut, gross, minOut, recipient, treasury);
        if (_balance(tokenOut) != beforeOutput) revert UnsupportedToken();
        emit Swapped(msg.sender, recipient, tokenIn, tokenOut, amountIn, amountOut);
    }

    function _payOutput(address token, uint256 gross, uint256 minOut, address recipient, address treasury) private returns (uint256 amountOut) {
        uint256 fee = feeFor(gross);
        amountOut = gross - fee;
        if (amountOut < minOut) revert SlippageExceeded();
        if (fee != 0) _pay(token, treasury, fee);
