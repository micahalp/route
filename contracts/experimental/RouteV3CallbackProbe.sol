...
contract RouteV3CallbackProbe is ReentrancyGuard {
    ...
    constructor(address f, address w) {
        if (f.code.length == 0 || w.code.length == 0) {
            revert InvalidRoute();
        }
        factory = IProbeV3Factory(f);
        wrapped = IProbeWrapped(w);
    }

    receive() external payable {
        if (msg.sender != address(wrapped)) {
            revert InvalidRoute();
        }
    }
    ...
    function swap(
        ...
        if (
            input == output || input.code.length == 0 || output.code.length == 0
                || recipient == address(0) || recipient == address(this) || fee == 0
                || fee >= 1_000_000
        ) {
            revert InvalidRoute();
        }
        if (
            amountIn == 0 || amountIn > uint256(type(int256).max) || minOut == 0
        ...
        activePool = pool;
        activeInput = input;
        activeAmount = amountIn;
        activeZeroForOne = input < output;
        IProbeV3Pool(pool)
            .swap(
                address(this),
                input < output,
    ...
    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data)
        external
    {
        if (msg.sender != activePool || activePool == address(0) || data.length != 0) {
            revert CallbackRejected();
        }
