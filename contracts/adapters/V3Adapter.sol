...
contract V3Adapter is BaseAdapter {
    ...
    constructor(string memory label, address router, address quoteContract, uint24[] memory fees) BaseAdapter(label, router) {
        if (quoteContract.code.length == 0 || fees.length == 0 || fees.length > 4) revert InvalidConfiguration();
    ...
    function quote(address tokenIn, address tokenOut, uint256 amountIn) external nonReentrant returns (uint256 amountOut, uint24 fee) {
        if (tokenIn == tokenOut || amountIn == 0) return (0, 0);
        for (uint256 i; i < feeTiers.length; ++i) {
            uint24 candidate = feeTiers[i];
            try IV3Quoter(quoter).quoteExactInputSingle{gas: 400_000}(IV3Quoter.Params(tokenIn, tokenOut, amountIn, candidate, 0))
                returns (uint256 result, uint160, uint32, uint256)
            {
                if (result > amountOut) { amountOut = result; fee = candidate; }
            } catch {}
        }
    }
    ...
    function _execute(address tokenIn, address tokenOut, uint256 amountIn, uint256 minOut, uint24 fee) internal override {
        ...
        IV3Router(dexRouter).exactInputSingle(IV3Router.Params(tokenIn, tokenOut, fee, address(this), amountIn, minOut, 0));
    }
}
