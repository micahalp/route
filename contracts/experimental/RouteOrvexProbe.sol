...
interface IOrvexManager {
    ...
    function swap(Key calldata key, Params calldata params, bytes calldata hookData)
        external
        returns (int256);
}
...
interface IOrvexWrapped is IERC20 {
...
contract RouteOrvexProbe is ReentrancyGuardTransient {
    ...
    function swap(
        ...
        if (
            block.chainid != 4663 || amountIn == 0 || amountIn > uint256(uint128(type(int128).max))
                || minimum == 0 || recipient == address(0) || recipient == address(this)
        ...
        if (
            MANAGER.codehash != 0xabcccba7b2c0339035fab45b93950740b990e1ff74a4c5d576fb82c65a93a29f
                || VAULT.codehash
        ...
        if (tokenIn == address(0)) {
            IOrvexWrapped(WETH).deposit{value: amountIn}();
        }
        ...
        if (
            address(this).balance != nativeBefore
                || IERC20(WETH).balanceOf(address(this)) != wrappedBefore
        ) {
            revert UnsupportedToken();
        }
        if (tokenIn != address(0) && IERC20(input).balanceOf(msg.sender) + amountIn != senderBefore)
        {
            revert UnsupportedToken();
        }
        ...
        if (
            tokenOut != address(0)
                && IERC20(output).balanceOf(recipient) != recipientBefore + amountOut
        ) {
            revert UnsupportedToken();
        }
    }
    ...
    function lockAcquired(bytes calldata data) external returns (bytes memory) {
        if (msg.sender != VAULT || pending == bytes32(0) || pending != keccak256(data)) {
            revert CallbackRejected();
        }
        pending = bytes32(0);
        (
        ...
        IOrvexManager.Key memory key =
            IOrvexManager.Key(WETH, USDG, HOOK, MANAGER, 8388608, bytes32(uint256(0x3c0042)));
        int256 delta = IOrvexManager(MANAGER)
            .swap(
                key,
        ...
        if (tokenIn == address(0)) {
            IERC20(input).safeTransfer(VAULT, amountIn);
        } else {
            IERC20(input).safeTransferFrom(payer, VAULT, amountIn);
        }
        if (IOrvexVault(VAULT).settle() != amountIn) {
            revert UnsupportedToken();
        }
        IOrvexVault(VAULT)
            .take(
                zeroForOne ? USDG : WETH,
                tokenOut == address(0) ? address(this) : recipient,
                amountOut
            );
