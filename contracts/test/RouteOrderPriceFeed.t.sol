...
contract PriceAggregatorFixture is IOrderAggregator {
    ...
    function set(int256 a, uint256 s, uint256 u) external { answer=a; since=s; updated=u; }
...
contract RouteOrderPriceFeedTest {
    ...
    function setUp() public {
        vm.warp(10000); token=new Token("M"); source=new PriceAggregatorFixture();
        uptime=new PriceAggregatorFixture(); uptime.set(0,100,100);
        reader=new RouteOrderPriceFeed(configs(),address(uptime),3600);
    }
    function testNormalizedPriceAndOriginalTimestamp() public view {
        (uint256 p,uint256 at)=reader.price(address(token)); require(p==2000 ether && at==10000);
    }
    function testUnknownTokenRejected() public {
        vm.expectRevert(RouteOrderPriceFeed.UnsupportedToken.selector);reader.price(address(123));
    }
    function testStaleZeroFutureNegativeRejected() public {
        int256[5] memory answers=[int256(1),int256(1),int256(1),int256(0),int256(-1)];
        uint256[5] memory times=[uint256(9939),uint256(0),uint256(10001),uint256(10000),uint256(10000)];
        for(uint256 i;i<5;++i) {
            source.set(answers[i],100,times[i]);
            vm.expectRevert(RouteOrderPriceFeed.InvalidPrice.selector);reader.price(address(token));
        }
        source.set(1,100,9940);reader.price(address(token));
    }
    function testSequencerDownUnknownUninitializedAndFutureRejected() public {
        ...
        uint256[5] memory starts=[uint256(100),uint256(100),uint256(0),uint256(10001),uint256(100)];
        uint256[5] memory reports=[uint256(100),uint256(100),uint256(0),uint256(10001),uint256(10001)];
        for(uint256 i;i<5;++i) {
            uptime.set(statuses[i],starts[i],reports[i]);
            vm.expectRevert(RouteOrderPriceFeed.SequencerUnavailable.selector);reader.price(address(token));
        }
    }
    function testRecoveryGraceBoundary() public {
        uptime.set(0,6400,6400);
        vm.expectRevert(RouteOrderPriceFeed.SequencerUnavailable.selector);reader.price(address(token));
        uptime.set(0,6399,6399);reader.price(address(token));
    }
    function testDecimalsChangeAndOverflowRejected() public {
        source.setDecimals(9);
        vm.expectRevert(RouteOrderPriceFeed.InvalidPrice.selector);reader.price(address(token));
        source.setDecimals(8);source.set(type(int256).max,100,10000);
        vm.expectRevert(RouteOrderPriceFeed.InvalidPrice.selector);reader.price(address(token));
    }
    ...
    function testFuzzNormalization(uint64 answer) public {
        ...
        (uint256 p,)=reader.price(address(token));require(p==uint256(answer)*1e10);
    }
}
