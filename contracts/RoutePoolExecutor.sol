        ...
        uint256 deadline, Branch[] calldata branches) external payable nonReentrant returns (uint256 amountOut)
    {
        ...
        for (uint256 b; b < branches.length; ++b) {
            ...
            for (uint256 h; h < branches[b].legs.length; ++h) {
                ...
                if (pairFactory == address(0) && poolFactory == address(0)) IERC20(current).forceApprove(spender, amount);
                ...
                if (pairFactory == address(0) && poolFactory == address(0)) IERC20(current).forceApprove(spender, 0);
                ...
                if ((pairFactory != address(0) || poolFactory != address(0)) && amount != expectedDirectOutput) revert UnsupportedToken();
