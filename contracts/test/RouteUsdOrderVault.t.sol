...
contract RouteUsdOrderVaultTest {
    ...
    function setUp() public {
        ...
        feed.set(2000 ether, 1000);
        vault = new Vault(address(adapter), address(feed), 60);
        market.mint(address(this), 1000 ether);
    ...
    function testUsdTakeProfitNotPoint113EthAndRepricesWhenEthMoves() public {
        ...
        (,,, minimum) = vault.execution(id, Vault.Leg.TakeProfit);
        require(minimum == 0.002825 ether);
        adapter.configure(minimum - 1, false, false);
        vm.expectRevert(Vault.MinimumNotMet.selector);
        vault.execute(id, Vault.Leg.TakeProfit, "");
        adapter.configure(minimum, false, false);
        vault.execute(id, Vault.Leg.TakeProfit, "");
    ...
    function testEntryUsdTargetRemainsFixedWhenFundingTokenChangesPrice() public {
        ...
        (,,, uint256 minimum) = vault.execution(id, Vault.Leg.Entry);
        require(minimum == 200 ether);
        feed.set(3000 ether, 1000);
        (,,, minimum) = vault.execution(id, Vault.Leg.Entry);
        require(minimum == 300 ether);
        adapter.configure(200 ether, false, false);
        vm.expectRevert(Vault.MinimumNotMet.selector);
        vault.execute(id, Vault.Leg.Entry, "");
        adapter.configure(300 ether, false, false);
        vault.execute(id, Vault.Leg.Entry, "");
    }
    ...
    function testStopFloorUsesCurrentUsdPriceAndNamedKeeper() public {
        ...
        (,,, uint256 minimum) = vault.execution(id, Vault.Leg.StopLoss);
        require(minimum == 0.008955 ether);
        adapter.configure(minimum - 1, false, false);
        vm.prank(keeper);
        vm.expectRevert(Vault.MinimumNotMet.selector);
        vault.execute(id, Vault.Leg.StopLoss, "");
    ...
    function testBracketSizesExitToActualEntryAndInvalidatesSibling() public {
        ...
        vault.execute(id, Vault.Leg.TakeProfit, "");
        adapter.configure(250 ether, false, false);
        vault.execute(id, Vault.Leg.Entry, "");
        require(vault.getOrder(id).held == 250 ether);
        feed.set(4000 ether, 1000);
        (,,, uint256 minimum) = vault.execution(id, Vault.Leg.TakeProfit);
        require(minimum == 0.0070625 ether);
        adapter.configure(minimum, false, false);
        vault.execute(id, Vault.Leg.TakeProfit, "");
        vm.prank(keeper);
        vm.expectRevert(Vault.NotEligible.selector);
    ...
    function testStaleFutureZeroFeedFailsClosedButCancellationWorks() public {
        uint256 id = vault.create(terms(Vault.Kind.LimitBuy));
        feed.set(2000 ether, 939);
        vm.expectRevert(Vault.InvalidPrice.selector);
        vault.execution(id, Vault.Leg.Entry);
        vm.expectRevert(Vault.InvalidPrice.selector);
        vault.create(terms(Vault.Kind.LimitBuy));
        feed.set(2000 ether, 1001);
        vm.expectRevert(Vault.InvalidPrice.selector);
        vault.execution(id, Vault.Leg.Entry);
        feed.set(0, 1000);
        vm.expectRevert(Vault.InvalidPrice.selector);
        vault.execution(id, Vault.Leg.Entry);
        vault.cancel(id);
        require(weth.balanceOf(address(this)) == 100 ether);
    }

    function testPartialSpendRedirectAndReplayRejected() public {
        uint256 id = vault.create(terms(Vault.Kind.LimitBuy));
        adapter.configure(200 ether, true, false);
        vm.expectRevert(Vault.TransferMismatch.selector);
        vault.execute(id, Vault.Leg.Entry, "");
        adapter.configure(200 ether, false, true);
        vm.expectRevert(Vault.MinimumNotMet.selector);
        vault.execute(id, Vault.Leg.Entry, "");
        adapter.configure(200 ether, false, false);
    ...
    function testExpiryAndCancelAfterEntry() public {
        ...
        vault.execute(id, Vault.Leg.Entry, "");
        vm.warp(2000);
        feed.set(2000 ether, 2000);
        vm.expectRevert(Vault.NotEligible.selector);
        vault.execute(id, Vault.Leg.TakeProfit, "");
        vault.cancel(id);
        require(market.balanceOf(address(this)) == 1200 ether);
    }
}
