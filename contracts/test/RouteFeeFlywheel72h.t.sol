    ...
    function swap(
        ...
        if (reenter) {
            (bool ok,) = msg.sender
                .call(
                    abi.encodeCall(
                        RouteFeeFlywheel72h.execute, (0, 1 ether, 1, block.timestamp, branches)
                    )
                );
...
contract RouteFeeFlywheel72hTest {
    ...
    function terms() private view returns (RouteFeeFlywheel72h.Policy memory) {
        return
            RouteFeeFlywheel72h.Policy(2, 10 ether, 20 ether, 100 ether, 60, block.timestamp + 3600);
    }
    ...
    function testStartsPausedAndDeployerCannotActivate() public {
        ...
        vm.expectRevert(RouteFeeFlywheel72h.Unauthorized.selector);
        wheel.setPolicy(terms());
    }
    ...
    function testLowFloorRejectedBeforeClaim() public {
        activate();
        escrow.credit(address(wheel), 1 ether);
        vm.expectRevert(RouteFeeFlywheel72h.SlippageExceeded.selector);
        run(0, 1 ether, 1);
    ...
    function testDuplicateAndEarlyExecutionRejected() public {
        ...
        run(0, 1 ether, 65 ether);
        escrow.credit(address(wheel), 1 ether);
        vm.expectRevert(RouteFeeFlywheel72h.InvalidBatch.selector);
        run(0, 1 ether, 65 ether);
        vm.expectRevert(RouteFeeFlywheel72h.InvalidBatch.selector);
    ...
    function testExpiry() public {
        ...
        vm.expectRevert(RouteFeeFlywheel72h.Inactive.selector);
        run(0, 1 ether, 65 ether);
    }

    function test72HourPolicyAcceptedAndThenExpires() public {
        require(wheel.MAX_POLICY_DURATION() == 72 hours);
        RouteFeeFlywheel72h.Policy memory p = terms();
        p.expiresAt = block.timestamp + 72 hours;
        vm.prank(address(safe));
        wheel.setPolicy(p);
    ...
    function testPolicyOver72HoursRejected() public {
        RouteFeeFlywheel72h.Policy memory p = terms();
        p.expiresAt = block.timestamp + 72 hours + 1;
