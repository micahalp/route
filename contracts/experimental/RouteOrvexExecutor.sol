...
contract RouteOrvexExecutor is ReentrancyGuardTransient, OrvexVaultLeg {
    ...
    constructor(
        ...
        _assertOrvexRuntime();
        if (
            wrapped != orvexWeth || v4 == orvexManager || fastV3 == orvexManager
                || fastV2 == orvexManager || ramses == orvexManager
        ) {
            revert InvalidRoute();
        }
        ...
        if (!allowedAdapter[orvexManager]) {
            revert InvalidRoute();
        }
    ...
    function swap(
        ...
        for (uint256 b; b < branches.length; ++b) {
            ...
            for (uint256 h; h < branch.legs.length; ++h) {
                ...
                if (leg.adapter == orvexManager && leg.fee != 0) {
                    revert InvalidRoute();
                }
        ...
        for (uint256 b; b < branches.length; ++b) {
            ...
            for (uint256 h; h < branches[b].legs.length; ++h) {
                ...
                if (
                    (pairFactory != address(0)
                            || poolFactory != address(0)
                            || leg.adapter == orvexManager) && amount != expectedDirectOutput
                ) {
                    revert UnsupportedToken();
                }
                if (amount == 0) {
