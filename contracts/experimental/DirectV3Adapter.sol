...
interface IDirectV3Factory {
    function getPool(address, address, uint24) external view returns (address);
}

interface IDirectV3Quoter is IV3Quoter {
...
contract DirectV3Adapter is IRouteAdapter, ReentrancyGuardTransient {
    ...
    constructor(
        ...
        if (
            bytes(label).length == 0 || poolFactory.code.length == 0
                || quoteContract.code.length == 0 || fees.length == 0 || fees.length > 4
                || IDirectV3Quoter(quoteContract).factory() != poolFactory
        ) {
    ...
    {
        if (tokenIn == tokenOut || amountIn == 0 || amountIn > uint256(type(int256).max)) {
            return (0, 0);
        }
        ...
        returns (uint256 amountOut)
    {
        ...
        expectedPool = pool;
        expectedInput = tokenIn;
        expectedAmount = amountIn;
        expectedDirection = direction;
        (int256 delta0, int256 delta1) = IDirectV3Pool(pool)
            .swap(
                address(this),
                direction,
                int256(amountIn),
        ...
        if (expectedPool != address(0) || expectedAmount != 0) {
            revert CallbackRejected();
        }
        delete expectedInput;
        delete expectedDirection;
        int256 spent = direction ? delta0 : delta1;
        int256 received = direction ? delta1 : delta0;
        if (spent != int256(amountIn) || received >= 0 || received == type(int256).min) {
            revert InvalidSwap();
        }
        ...
        if (amountOut != uint256(-received) || amountOut == 0 || amountOut < minOut) {
            revert InvalidSwap();
        }
        uint256 recipientBefore = output.balanceOf(msg.sender);
        output.safeTransfer(msg.sender, amountOut);
        if (
            output.balanceOf(msg.sender) != recipientBefore + amountOut
    ...
    function _pay(int256 delta0, int256 delta1) private {
        ...
        if (owed <= 0 || uint256(owed) != expectedAmount || other >= 0) {
            revert CallbackRejected();
        }
        ...
        if (input.balanceOf(msg.sender) != beforeBalance + uint256(owed)) {
            revert UnsupportedToken();
        }
    }
}
