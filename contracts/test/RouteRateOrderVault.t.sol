...
contract RouteRateOrderVaultTest {
    ...
    function setUp() public {
        ...
        vault = new Vault(address(adapter));
    ...
    function terms(Vault.Kind kind) internal view returns (Vault.Terms memory t) {
        ...
        t.amount = 10 ether;
        t.expiresAt = 2000;
        if (kind == Vault.Kind.LimitBuy || kind == Vault.Kind.Bracket) {
            t.entryMinimum = 20 ether;
        }
        if (kind == Vault.Kind.TakeProfit || kind == Vault.Kind.Bracket) {
            t.takeProfitRate = 1 ether;
        }
        if (kind == Vault.Kind.StopLoss || kind == Vault.Kind.Bracket) {
            t.stopTriggerRate = 0.4 ether;
            t.stopMinimumRate = 0.3 ether;
            t.keeper = keeper;
        }
    }
    ...
    function testLimitNeedsOutputNotAnOracle() public {
        ...
        adapter.configure(19 ether, false, false);
        vm.expectRevert(Vault.MinimumNotMet.selector);
        vault.execute(id, Vault.Leg.Entry, "");
        require(vault.getOrder(id).state == Vault.State.Pending);
        adapter.configure(20 ether, false, false);
        vault.execute(id, Vault.Leg.Entry, "");
    ...
    function testPermissionlessTakeProfitAtAuthorizedRate() public {
        ...
        vault.execute(id, Vault.Leg.TakeProfit, "");
    ...
    function testOnlyNamedKeeperCanTriggerStopEvenOwnerCannot() public {
        ...
        vm.expectRevert(Vault.UnauthorizedKeeper.selector);
        vault.execute(id, Vault.Leg.StopLoss, "");
        vm.prank(address(123));
        vm.expectRevert(Vault.UnauthorizedKeeper.selector);
        vault.execute(id, Vault.Leg.StopLoss, "");
        adapter.configure(3 ether, false, false);
        vm.prank(keeper);
        vault.execute(id, Vault.Leg.StopLoss, "");
    ...
    function testKeeperCannotWaiveFloorOrChangeRecipient() public {
        uint256 id = vault.create(terms(Vault.Kind.StopLoss));
        adapter.configure(2 ether, false, false);
        vm.prank(keeper);
        vm.expectRevert(Vault.MinimumNotMet.selector);
        vault.execute(id, Vault.Leg.StopLoss, "");
        ...
        vm.expectRevert(Vault.MinimumNotMet.selector);
        vault.execute(id, Vault.Leg.StopLoss, "");
        require(vault.getOrder(id).held == 10 ether);
        vault.cancel(id);
        require(market.balanceOf(address(this)) == 100 ether);
    }

    function testBracketActualProceedsAndTakeProfitInvalidatesStop() public {
        uint256 id = vault.create(terms(Vault.Kind.Bracket));
        vm.prank(keeper);
        vm.expectRevert(Vault.NotEligible.selector);
        vault.execute(id, Vault.Leg.StopLoss, "");
        adapter.configure(25 ether, false, false);
        vault.execute(id, Vault.Leg.Entry, "");
        require(vault.getOrder(id).held == 25 ether);
        adapter.configure(24 ether, false, false);
        vm.expectRevert(Vault.MinimumNotMet.selector);
        vault.execute(id, Vault.Leg.TakeProfit, "");
        adapter.configure(25 ether, false, false);
        vault.execute(id, Vault.Leg.TakeProfit, "");
    ...
    function testBracketStopInvalidatesTakeProfit() public {
        uint256 id = vault.create(terms(Vault.Kind.Bracket));
        vault.execute(id, Vault.Leg.Entry, "");
        adapter.configure(6 ether, false, false);
        vm.prank(keeper);
        vault.execute(id, Vault.Leg.StopLoss, "");
        vm.expectRevert(Vault.NotEligible.selector);
        vault.execute(id, Vault.Leg.TakeProfit, "");
    ...
    function testCancelAfterEntryAndExpiryReturnsOnlyHeld() public {
        uint256 id = vault.create(terms(Vault.Kind.Bracket));
        vault.execute(id, Vault.Leg.Entry, "");
        market.mint(address(vault), 7 ether);
        vm.warp(2000);
        vm.expectRevert(Vault.NotEligible.selector);
        vault.execute(id, Vault.Leg.TakeProfit, "");
        vault.cancel(id);
        require(
            market.balanceOf(address(this)) == 120 ether
                && market.balanceOf(address(vault)) == 7 ether
        );
        vm.expectRevert(Vault.NotEligible.selector);
        vault.cancel(id);
    }
    ...
    function testKeeperCannotCancelOrSpendOtherOrder() public {
        ...
        vault.cancel(id);
        uint256 other = vault.create(terms(Vault.Kind.LimitBuy));
        vault.execute(other, Vault.Leg.Entry, "");
    ...
    function testInvalidKeeperFloorAndBracketRejected() public {
        Vault.Terms memory t = terms(Vault.Kind.StopLoss);
        t.keeper = address(0);
        vm.expectRevert(Vault.InvalidOrder.selector);
        vault.create(t);
        t = terms(Vault.Kind.StopLoss);
        t.stopMinimumRate = t.stopTriggerRate + 1;
        vm.expectRevert(Vault.InvalidOrder.selector);
        vault.create(t);
        t = terms(Vault.Kind.LimitBuy);
        t.keeper = keeper;
        vm.expectRevert(Vault.InvalidOrder.selector);
        vault.create(t);
        t = terms(Vault.Kind.Bracket);
        t.stopTriggerRate = 0.5 ether;
        vm.expectRevert(Vault.InvalidOrder.selector);
        vault.create(t);
    }
    ...
    function testPartialSpendAndReentryRejected() public {
        ...
        vm.expectRevert(Vault.TransferMismatch.selector);
        vault.execute(id, Vault.Leg.Entry, "");
        adapter.configure(20 ether, false, false);
    ...
    function testFuzzMinimumRoundsUp(uint96 a, uint96 r) public {
        ...
        Vault.Terms memory t = terms(Vault.Kind.TakeProfit);
        t.amount = a;
        t.takeProfitRate = r;
        market.mint(address(this), a);
    ...
    function testTaxedDepositRejectedWithoutCustodyOrOrder() public {
        ...
        Vault.Terms memory t = terms(Vault.Kind.LimitBuy);
