...
contract MockDex {
    ...
    function setFailure(bool q, bool s) external {
        failQuote = q;
        failSwap = s;
    }

    function setExecution(uint256 bps) external {
        executionBps = bps;
    }
...
contract ReenterNative {
    ...
    receive() external payable {
        try router.quote(address(0), address(1), 1) {
            revert("reentered");
        } catch {
            prevented = true;
        }
...
contract RouteTest {
    ...
    function setUp() public {
        ...
        dex1 = new MockDex();
        dex2 = new MockDex();
        v1 = new V2Adapter("exchange one", address(dex1));
        v2 = new V2Adapter("exchange two", address(dex2));
    ...
    function testBestDirectAcrossDexes() public {
        ...
        require(
            q.amountOut == 30 ether && q.first.adapter == address(v2)
                && q.intermediate == address(0)
        );
    }
    ...
    function testCrossDexTwoHop() public {
        ...
        vm.prank(user);
        router.swap(address(a), address(b), 10 ether, 39 ether, user, block.timestamp);
    ...
    function testRequotesAtExecution() public {
        ...
        vm.prank(user);
        uint256 result =
            router.swap(address(a), address(b), 10 ether, 29 ether, user, block.timestamp);
        require(result == 40 ether && b.balanceOf(user) == beforeOut + result);
    }

    function testExpiredReverts() public {
        vm.warp(100);
        vm.expectRevert(Route.DeadlineExpired.selector);
        vm.prank(user);
        router.swap(address(a), address(b), 1 ether, 1, user, 99);
    }
    ...
    function testSlippageRevertsAtomically() public {
        ...
        vm.prank(user);
        router.swap(address(a), address(b), 1 ether, 4 ether, user, block.timestamp);
    ...
    function testApprovalsClearedAndDonationUntouched() public {
        ...
        b.mint(address(router), 9);
        a.mint(address(v2), 11);
        b.mint(address(v2), 13);
        vm.prank(user);
        router.swap(address(a), address(b), 1 ether, 2 ether, user, block.timestamp);
    ...
    function testNoRoute() public {
        vm.expectRevert(Route.NoRoute.selector);
        router.quote(address(b), address(a), 1 ether);
    }
    ...
    function testNativeIn() public {
        ...
        uint256 beforeOut = b.balanceOf(user);
        vm.prank(user);
        router.swap{value: 1 ether}(address(0), address(b), 1 ether, 1 ether, user, block.timestamp);
    ...
    function testNativeOut() public {
        ...
        vm.prank(user);
        router.swap(address(a), address(0), 1 ether, 1 ether, user, block.timestamp);
    ...
    function testNativeRecipientRejectionRollsBack() public {
        ...
        uint256 beforeIn = a.balanceOf(user);
        vm.expectRevert(Route.NativeTransferFailed.selector);
        vm.prank(user);
    ...
    function testReentrancyPrevented() public {
        ...
        vm.prank(user);
        router.swap(address(a), address(0), 1 ether, 1 ether, address(receiver), block.timestamp);
        require(receiver.prevented());
    }

    function testWrongValueRejected() public {
        vm.expectRevert(Route.InvalidValue.selector);
        vm.prank(user);
        router.swap{value: 1}(address(a), address(b), 1 ether, 1, user, block.timestamp);
    }

    function testSameTokenAndZeroAmountRejected() public {
        vm.expectRevert(Route.InvalidPair.selector);
        router.quote(address(a), address(a), 1);
        vm.expectRevert(Route.InvalidPair.selector);
        router.quote(address(0), address(weth), 1);
        vm.expectRevert(Route.InvalidAmount.selector);
        router.quote(address(a), address(b), 0);
    }
    ...
    function testZeroMinimumAndBadRecipientRejected() public {
        ...
        vm.expectRevert(Route.InvalidRecipient.selector);
        vm.prank(user);
    ...
    function testTaxedInputRejected() public {
        ...
        taxed.mint(user, 10 ether);
        vm.prank(user);
        taxed.approve(address(router), 10 ether);
        vm.expectRevert(Route.UnsupportedToken.selector);
        vm.prank(user);
    ...
    function testTaxedOutputRejected() public {
        TaxToken taxed = new TaxToken();
        taxed.mint(address(dex1), 10 ether);
        dex1.setRate(address(a), address(taxed), 1e18);
        vm.expectRevert(BaseAdapter.UnsupportedToken.selector);
        vm.prank(user);
    ...
    function testInvalidConfigurationRejected() public {
        ...
        venues[0] = address(v1);
        venues[1] = address(v1);
        vm.expectRevert(Route.InvalidConfiguration.selector);
        new Route(address(weth), venues, new address[](0));
    }
    ...
    function testFuzzConservation(uint96 seed) public {
        ...
        uint256 beforeOut = b.balanceOf(user);
        vm.prank(user);
        uint256 result = router.swap(address(a), address(b), amount, amount, user, block.timestamp);
        require(
            result == amount * 3 && a.balanceOf(user) == beforeIn - amount
                && b.balanceOf(user) == beforeOut + result
        );
