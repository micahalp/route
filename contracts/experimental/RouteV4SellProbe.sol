...
contract RouteV4SellProbe is ReentrancyGuard {
    ...
    event Swapped(address indexed sender, address indexed recipient, uint256 amountIn, uint256 amountOut);

    constructor(address manager, address token, address hooks, bytes32 expectedPoolId) {
        if (manager.code.length == 0 || token.code.length == 0 || hooks.code.length == 0) revert InvalidSwap();
        poolManager = IRoutePoolManager(manager); tokenIn = IERC20(token); hook = hooks;
        poolId = keccak256(abi.encode(address(0), token, uint24(0x800000), int24(1), hooks));
        if (poolId != expectedPoolId) revert InvalidSwap();
        managerHash = manager.codehash; hookHash = hooks.codehash;
    }

    receive() external payable { if (msg.sender != address(poolManager)) revert UnauthorizedCallback(); }
        ...
        external nonReentrant returns (uint256 amountOut)
    {
        ...
        if (amountIn == 0 || amountIn > uint256(uint128(type(int128).max)) || minimum == 0 ||
            ...
            address(poolManager).codehash != managerHash || hook.codehash != hookHash) revert InvalidSwap();
        ...
        if (beforePayer < amountIn || tokenIn.balanceOf(msg.sender) != beforePayer - amountIn ||
            tokenIn.balanceOf(address(this)) != beforeHeld || address(this).balance != beforeNative + amountOut) revert UnsupportedToken();
        (bool ok,) = recipient.call{value: amountOut}("");
        if (!ok) revert InvalidSwap();
        if (address(this).balance != beforeNative) revert UnsupportedToken();
    ...
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        ...
        pending = bytes32(0);
        (address payer, uint256 amountIn, uint256 minimum) = abi.decode(data, (address, uint256, uint256));
        IRoutePoolManager.PoolKey memory key = IRoutePoolManager.PoolKey(address(0), address(tokenIn), 0x800000, 1, hook);
        int256 delta = poolManager.swap(key, IRoutePoolManager.SwapParams(false, -int256(amountIn), 1461446703485210103287273052203988822378723970341), "");
        int128 received = int128(delta >> 128); int128 paid = int128(delta);
