...
import {RouteFeeFlywheel72h, IFlywheelExecutor, IFlywheelFactory} from "../experimental/RouteFeeFlywheel72h.sol";
...
interface FlyRealSafe {
    ...
    function execTransaction(address,uint256,bytes calldata,uint8,uint256,uint256,uint256,address,address payable,bytes calldata) external payable returns(bool);
}
...
contract RouteFeeFlywheel72hForkTest {
    ...
    // Historical funded operator fixture: RPC cannot read the new, unfunded bot at this old block.
    // Production constructor arguments are separately pinned by the deployment preparer.
    ...
    function safeCall(bytes memory data) private {
        ...
        bytes32 hash = safe.getTransactionHash(address(wheel),0,data,0,0,0,0,address(0),address(0),safe.nonce());
        vm.prank(OWNER1); safe.approveHash(hash);
        vm.prank(OWNER2); safe.approveHash(hash);
        bytes memory signatures = abi.encodePacked(bytes32(uint256(uint160(OWNER1))),bytes32(0),uint8(1),
            bytes32(uint256(uint160(OWNER2))),bytes32(0),uint8(1));
        require(safe.execTransaction(address(wheel),0,data,0,0,0,0,address(0),payable(address(0)),signatures), "Safe transaction failed");
    }
    function testForkRealSafeEscrowExecutorAndRecipientHandover() public {
        ...
        vm.prank(OPERATOR);
        wheel = new RouteFeeFlywheel72h(SAFE,OPERATOR,TOKEN,ESCROW,FACTORY,EXECUTOR);
        ...
        RouteFeeFlywheel72h.Policy memory p = RouteFeeFlywheel72h.Policy(0.0001 ether,0.0001 ether,
            0.0001 ether,300000 ether,60,block.timestamp + 300);
        safeCall(abi.encodeCall(RouteFeeFlywheel72h.setPolicy,(p)));
        ...
        legs[0] = IFlywheelExecutor.Leg(0xd5Cdc1eFA4a625504b9E8792ee42776F16cDf09A,TOKEN,9000,90,address(0),true);
        branches[0] = IFlywheelExecutor.Branch(0.000065 ether,legs);
        uint256 ethBefore = SAFE.balance; uint256 tokensBefore = IERC20(TOKEN).balanceOf(SAFE);
        vm.prank(OPERATOR);
        ...
        safeCall(abi.encodeCall(RouteFeeFlywheel72h.pause,()));
        safeCall(abi.encodeCall(RouteFeeFlywheel72h.handoverFeesToTreasury,()));
    }
}
