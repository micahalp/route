...
interface ILeanV3Factory { function getPool(address, address, uint24) external view returns (address); }
interface ILeanV3Pool { function swap(address, bool, int256, uint160, bytes calldata) external returns (int256, int256); }
interface ILeanV3Router { function factory() external view returns (address); function WETH9() external view returns (address); }
interface ILeanV2Pair {
    function getReserves() external view returns (uint112, uint112, uint32);
    function swap(uint256, uint256, address, bytes calldata) external;
}

import {OrvexVaultLeg} from "./OrvexVaultLeg.sol";

/// @notice Experimental six-venue mixed settlement; not deployed or active.
/// @dev Based on the unchanged five-venue RoutePoolExecutor at 15d7adc.
/// @dev Separate release source preserves historical active source/receipts.
/// @dev Retains adapter fallback but avoids extra token custody hops for fixed V2/V3/Ramses routers.
/// @dev Separate full executor; deployed RouteRamsesExecutor source remains unchanged.
/// @dev Only verified canonical V3 pools gain direct callback payment; every fallback is retained.
/// @dev Immutable adapters; no owner, upgrades, arbitrary calls or retained allowances.
contract RouteOrvexExecutor is ReentrancyGuardTransient, OrvexVaultLeg {
    ...
    constructor(address wrapped, address[] memory adapters, address v4, address fastV3, address fastV2, address ramses, address factory) {
        ...
        _assertOrvexRuntime();
        if (wrapped != orvexWeth || v4 == orvexManager || fastV3 == orvexManager || fastV2 == orvexManager || ramses == orvexManager) revert InvalidRoute();
        ...
        if (!allowedAdapter[orvexManager]) revert InvalidRoute();
        ...
        fastV3Router = target;
        address poolFactory;
        if (block.chainid == 4663 && target == 0xCaf681a66D020601342297493863E78C959E5cb2 &&
            target.codehash == 0x6f36c378e272c6324c48f045182bcb54bd8ad654cf9ebd42e8893d52c4cb25dc) {
            poolFactory = ILeanV3Router(target).factory();
            if (poolFactory != 0x1f7d7550B1b028f7571E69A784071F0205FD2EfA ||
                poolFactory.codehash != 0xec72b1abd1f2faee020cfea9c646bd8994f9fb389054f6e574f103a895091739 ||
                ILeanV3Router(target).WETH9() != wrapped) revert InvalidRoute();
        }
    ...
    function _v3PoolFactory() internal view virtual returns (address) { return fastV3PoolFactory; }

    function _poolSwap(address factory, address input, address output, uint24 fee, uint256 amount) private returns (uint256 out) {
        ...
        callbackPool = pool; callbackInput = input; callbackAmount = amount; callbackZeroForOne = input < output;
        ILeanV3Pool(pool).swap(address(this), input < output, int256(amount), input < output
        ...
        out = callbackOutput;
        delete callbackInput; delete callbackOutput; delete callbackZeroForOne;
    }

    function uniswapV3SwapCallback(int256 amount0Delta, int256 amount1Delta, bytes calldata data) external {
        if (msg.sender != callbackPool || callbackPool == address(0) || data.length != 0) revert CallbackRejected();
        int256 owed = callbackZeroForOne ? amount0Delta : amount1Delta;
        int256 received = callbackZeroForOne ? amount1Delta : amount0Delta;
        if (owed <= 0 || received >= 0 || received == type(int256).min || uint256(owed) != callbackAmount) revert CallbackRejected();
        callbackOutput = uint256(-received);
        address token = callbackInput;
        delete callbackPool; delete callbackAmount;
        ...
        uint256 deadline, Branch[] calldata branches) external payable nonReentrant returns (uint256 amountOut)
    {
        ...
        for (uint256 b; b < branches.length; ++b) {
            ...
            for (uint256 h; h < branch.legs.length; ++h) {
                ...
                if (leg.adapter == orvexManager && leg.fee != 0) revert InvalidRoute();
        ...
        for (uint256 b; b < branches.length; ++b) {
            ...
            for (uint256 h; h < branches[b].legs.length; ++h) {
                ...
                if (pairFactory == address(0) && poolFactory == address(0) && leg.adapter != orvexManager) IERC20(current).forceApprove(spender, amount);
                if (leg.adapter == orvexManager) {
                    expectedDirectOutput = _orvexSwap(current, leg.tokenOut, amount);
                } else if (pairFactory != address(0)) {
                    ...
                    expectedDirectOutput = _poolSwap(poolFactory, current, leg.tokenOut, leg.fee, amount);
                ...
                if (pairFactory == address(0) && poolFactory == address(0) && leg.adapter != orvexManager) IERC20(current).forceApprove(spender, 0);
                ...
                if ((pairFactory != address(0) || poolFactory != address(0) || leg.adapter == orvexManager) && amount != expectedDirectOutput) revert UnsupportedToken();
