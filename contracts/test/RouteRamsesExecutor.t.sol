...
contract RamsesMock {
    ...
    function setZeroOutput() external {
        zeroOutput = true;
    }
    ...
    function exactInputSingle(IRamsesRouter.Params calldata p) external payable returns (uint256) {
        ...
        require(
            p.sqrtPriceLimitX96 == 0 && p.amountOutMinimum == 1 && p.recipient == msg.sender
                && msg.value == 0
        );
        require(IERC20(p.tokenIn).allowance(msg.sender, address(this)) == p.amountIn);
        require(
            IERC20(p.tokenIn).transferFrom(msg.sender, address(this), p.amountIn * fillBps / 10000)
        );
...
contract RouteRamsesExecutorTest is RouteExecutorTest {
    ...
    function deployExecutor(address wrapped, address[] memory adapters)
        internal
        override
        returns (RouteExecutor)
    {
        ramses = new RamsesMock(wrapped);
        factory = new RamsesFactoryMock(address(ramses));
    ...
    function setUp() public virtual override {
        super.setUp();
        a.mint(address(ramses), 1e32);
        b.mint(address(ramses), 1e32);
        c.mint(address(ramses), 1e32);
        weth.mint(address(ramses), 1e32);
    }
    ...
    function rplan(uint256 amount, address output)
        internal
        view
        returns (RouteExecutor.Branch[] memory p)
    {
        p = direct(amount);
        p[0].legs[0] = rleg(output);
    }
    ...
    function testRamsesActualOutputAndDonations() public {
        ...
        uint256 out = router.swap(
            ...
            user,
            block.timestamp,
            rplan(1 ether, address(b))
        );
        require(
            out == 3 ether && beforeIn - a.balanceOf(user) == 1 ether
                && b.balanceOf(user) - beforeOut == out
        );
    ...
    function testRamsesNativeInput() public {
        uint256 beforeOut = b.balanceOf(user);
        vm.prank(user);
        uint256 out = router.swap{value: 1 ether}(
            address(0),
            address(b),
            1 ether,
            3 ether,
        ...
        require(
            out == 3 ether && b.balanceOf(user) - beforeOut == out && address(router).balance == 0
    ...
    function testRamsesNativeOutput() public {
        ...
        uint256 out = router.swap(
            ...
            user,
            block.timestamp,
            rplan(1 ether, address(weth))
        );
        require(out == 3 ether && user.balance - beforeOut == out && address(router).balance == 0);
        cleared();
    }
    ...
    function testRamsesPartialInputRollsBack() public {
        ...
        router.swap(
            address(a), address(b), 1 ether, 1, user, block.timestamp, rplan(1 ether, address(b))
        );
    ...
    function testRamsesRejectsWrongFields() public {
        for (uint256 i; i < 6; i++) {
            ...
            if (i == 1) {
                p[0].legs[0].tickSpacing = -1;
            }
            if (i == 2) {
                p[0].legs[0].tickSpacing = 0;
            }
            if (i == 3) {
                p[0].legs[0].tickSpacing = 32768;
            }
            if (i == 4) {
                p[0].legs[0].hooks = address(1);
            }
            if (i == 5) {
                p[0].legs[0].nativePool = true;
            }
            vm.prank(user);
    ...
    function testRamsesRouterReentryBlocked() public {
        ...
        require(ramses.reentryPrevented());
        cleared();
    }
    ...
    function testRamsesNativeRecipientReentryBlocked() public {
        ...
        require(receiver.prevented() && address(receiver).balance == 3 ether);
        cleared();
    }
    ...
    function testRamsesTaxedRecipientOutputRejected() public {
        ...
        tax.mint(address(ramses), 100 ether);
        vm.prank(user);
        vm.expectRevert(RouteExecutor.UnsupportedToken.selector);
        router.swap(
            address(a),
    ...
    function testRamsesMixedV2AndAdapter() public {
        ...
        p[0] = rplan(1 ether, address(b))[0];
        p[0].legs = new RouteExecutor.Leg[](2);
        p[0].legs[0] = rleg(address(c));
        p[0].legs[1] = leg(address(v1), address(b));
        p[1] = direct(2 ether)[0];
        p[1].legs[0].adapter = address(v2);
        c.mint(address(router), 17);
        vm.prank(user);
        require(
            router.swap(address(a), address(b), 3 ether, 12 ether, user, block.timestamp, p)
                == 12 ether
        );
        require(
            c.balanceOf(address(router)) == 17 && c.allowance(address(router), address(dex1)) == 0
        );
        cleared();
    }

    function testRamsesMixedFastV3() public {
        FastMockV3 dex3 = new FastMockV3();
        uint24[] memory fees = new uint24[](1);
        fees[0] = 500;
        V3Adapter v3 = new V3Adapter("v3", address(dex3), address(dex3), fees);
        address[] memory adapters = new address[](3);
        adapters[0] = address(v1);
        adapters[1] = address(v3);
        adapters[2] = address(ramses);
        ...
        b.mint(address(dex3), 100 ether);
        RouteExecutor.Branch[] memory p = rplan(1 ether, address(b));
        p[0].legs = new RouteExecutor.Leg[](2);
        p[0].legs[0] = rleg(address(c));
        p[0].legs[1] = leg(address(v3), address(b));
        p[0].legs[1].fee = 500;
        vm.prank(user);
        require(
            router.swap(address(a), address(b), 1 ether, 9 ether, user, block.timestamp, p)
                == 9 ether
        );
        require(
            c.allowance(address(router), address(dex3)) == 0 && c.balanceOf(address(router)) == 0
        );
        cleared();
    }
    ...
    function testFuzzRamsesSplitExactInput(uint96 input, uint8 fraction) public {
        uint256 amount = uint256(input) + 100;
        uint256 left = amount * (uint256(fraction) % 99 + 1) / 100;
        RouteExecutor.Branch[] memory p = new RouteExecutor.Branch[](2);
        p[0] = rplan(left, address(b))[0];
        ...
        require(
            out == left * 3 + (amount - left) * 2 && beforeIn - a.balanceOf(user) == amount
                && b.balanceOf(user) - beforeOut == out
        );
        cleared();
    }
