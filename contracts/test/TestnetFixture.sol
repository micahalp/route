...
contract TestnetFixture {
    ...
    constructor() payable {
        ...
        wrapped = new Wrapped();
        dollar = new Token("TEST-USD");
        MockDex a = new MockDex();
        MockDex b = new MockDex();
        ...
        dollar.mint(address(a), 1000 ether);
        dollar.mint(address(b), 1000 ether);
        wrapped.deposit{value: msg.value}();
        wrapped.transfer(address(a), msg.value / 2);
        wrapped.transfer(address(b), msg.value / 2);
