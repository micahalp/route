...
import {RouteFeeFlywheel, IFlywheelExecutor} from "../experimental/RouteFeeFlywheel.sol";
...
contract FlyExecutor is IFlywheelExecutor {
    ...
    function swap(address input, address token, uint256 amount, uint256, address recipient, uint256,
        ...
        if (reenter) {
            (bool ok,) = msg.sender.call(abi.encodeCall(RouteFeeFlywheel.execute, (0, 1 ether, 1, block.timestamp, branches)));
...
contract RouteFeeFlywheelTest {
    ...
    function setUp() public {
        ...
        wheel = new RouteFeeFlywheel(address(safe), OPERATOR, address(token), address(escrow), address(factory), address(executor));
        vm.deal(address(escrow), 1000 ether);
    }
    function terms() private view returns (RouteFeeFlywheel.Policy memory) {
        return RouteFeeFlywheel.Policy(2, 10 ether, 20 ether, 100 ether, 60, block.timestamp + 3600);
    }
    ...
    function testStartsPausedAndDeployerCannotActivate() public {
        ...
        vm.expectRevert(RouteFeeFlywheel.Unauthorized.selector); wheel.setPolicy(terms());
    }
    ...
    function testLowFloorRejectedBeforeClaim() public {
        activate(); escrow.credit(address(wheel), 1 ether);
        vm.expectRevert(RouteFeeFlywheel.SlippageExceeded.selector); run(0, 1 ether, 1);
    ...
    function testRedirectRollsBackEverything() public {
        ...
        vm.expectRevert(RouteFeeFlywheel.SlippageExceeded.selector); run(0, 1 ether, 65 ether);
    ...
    function testDuplicateAndEarlyExecutionRejected() public {
        ...
        vm.expectRevert(RouteFeeFlywheel.InvalidBatch.selector); run(0, 1 ether, 65 ether);
        vm.expectRevert(RouteFeeFlywheel.InvalidBatch.selector); run(1, 1 ether, 65 ether);
        vm.warp(block.timestamp + 60); run(1, 1 ether, 65 ether);
    }
    function testExpiry() public {
        activate(); vm.warp(block.timestamp + 3601);
        vm.expectRevert(RouteFeeFlywheel.Inactive.selector); run(0, 1 ether, 65 ether);
    }
    function testBudgetAndBatchLimit() public {
        RouteFeeFlywheel.Policy memory p = terms(); p.remainingClaimsBudget = 1 ether;
        vm.prank(address(safe)); wheel.setPolicy(p);
        vm.expectRevert(RouteFeeFlywheel.InvalidBatch.selector); run(0, 2 ether, 130 ether);
        vm.expectRevert(RouteFeeFlywheel.InvalidBatch.selector); run(0, 11 ether, 715 ether);
    }
    function testOperatorCannotRecoverOrRedirect() public {
        vm.expectRevert(RouteFeeFlywheel.Unauthorized.selector); vm.prank(OPERATOR); wheel.recover(address(0), true);
        vm.expectRevert(RouteFeeFlywheel.Unauthorized.selector); vm.prank(OPERATOR); wheel.handoverFeesToTreasury();
    }
    ...
    function testRecoveryRequiresPauseAndDonationsDoNotChangeSplit() public {
        ...
        vm.expectRevert(RouteFeeFlywheel.Inactive.selector); vm.prank(address(safe)); wheel.recover(address(0), false);
