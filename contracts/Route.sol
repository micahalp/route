...
contract Route is ReentrancyGuard {
    ...
    struct Leg { address adapter; uint24 fee; }
    struct Quote { uint256 amountOut; address intermediate; Leg first; Leg second; }
    ...
    event Swapped(address indexed sender, address indexed recipient, address indexed tokenIn, address tokenOut,
        uint256 amountIn, uint256 amountOut, address intermediate, address firstAdapter, uint24 firstFee, address secondAdapter, uint24 secondFee);

    constructor(address wrapped, address[] memory venues, address[] memory bridges) {
        if (wrapped.code.length == 0 || venues.length == 0 || venues.length > 6 || bridges.length > 3) revert InvalidConfiguration();
        for (uint256 i; i < venues.length; ++i) {
            if (venues[i].code.length == 0) revert InvalidConfiguration();
            for (uint256 j; j < i; ++j) if (venues[j] == venues[i]) revert InvalidConfiguration();
        }
        for (uint256 i; i < bridges.length; ++i) {
            if (bridges[i].code.length == 0) revert InvalidConfiguration();
            for (uint256 j; j < i; ++j) if (bridges[j] == bridges[i]) revert InvalidConfiguration();
        }
    ...
    receive() external payable {
        if (msg.sender != address(wrappedNative)) revert InvalidValue();
    }

    function configuration() external view returns (address wrapped, address[] memory venues, address[] memory bridges) {
    ...
    function quote(address tokenIn, address tokenOut, uint256 amountIn) external nonReentrant returns (Quote memory) {
    ...
    function swap(address tokenIn, address tokenOut, uint256 amountIn, uint256 minOut, address recipient, uint256 deadline)
        external payable nonReentrant returns (uint256 amountOut)
    {
        ...
        (address input, address output) = _pair(tokenIn, tokenOut, amountIn);
        if (msg.value != (tokenIn == address(0) ? amountIn : 0)) revert InvalidValue();
        ...
        if (IERC20(input).balanceOf(address(this)) != inputBefore + amountIn) revert UnsupportedToken();

        Quote memory best = _find(input, output, amountIn);
        if (best.amountOut < minOut) revert SlippageExceeded(best.amountOut, minOut);
        if (best.intermediate == address(0)) {
            ...
            if (IERC20(best.intermediate).balanceOf(address(this)) != bridgeBefore) revert UnsupportedToken();
        }
        if (IERC20(input).balanceOf(address(this)) != inputBefore) revert UnsupportedToken();
        ...
        if (tokenOut == address(0)) {
            ...
            if (IERC20(output).balanceOf(recipient) != recipientBefore + amountOut) revert UnsupportedToken();
        }
        emit Swapped(msg.sender, recipient, tokenIn, tokenOut, amountIn, amountOut, best.intermediate,
            best.first.adapter, best.first.fee, best.second.adapter, best.second.fee);
    }

    function _pair(address tokenIn, address tokenOut, uint256 amountIn) private view returns (address input, address output) {
        input = tokenIn == address(0) ? address(wrappedNative) : tokenIn;
        output = tokenOut == address(0) ? address(wrappedNative) : tokenOut;
        if (input == output || input.code.length == 0 || output.code.length == 0) revert InvalidPair();
        if (amountIn == 0) revert InvalidAmount();
    }

    function _find(address input, address output, uint256 amountIn) private returns (Quote memory best) {
        (best.amountOut, best.first) = _edge(input, output, amountIn);
        for (uint256 i; i < connectors.length; ++i) {
            address bridge = connectors[i];
            if (bridge == input || bridge == output) continue;
            (uint256 firstOut, Leg memory first) = _edge(input, bridge, amountIn);
            if (firstOut == 0) continue;
            (uint256 lastOut, Leg memory second) = _edge(bridge, output, firstOut);
            if (lastOut > best.amountOut) best = Quote(lastOut, bridge, first, second);
        }
        if (best.amountOut == 0) revert NoRoute();
    }

    function _edge(address input, address output, uint256 amountIn) private returns (uint256 bestOut, Leg memory leg) {
        for (uint256 i; i < adapters.length; ++i) {
            try IRouteAdapter(adapters[i]).quote{gas: ADAPTER_QUOTE_GAS}(input, output, amountIn)
                returns (uint256 candidate, uint24 fee)
            {
                if (candidate > bestOut) { bestOut = candidate; leg = Leg(adapters[i], fee); }
            } catch {}
        }
    }

    function _execute(Leg memory leg, address input, address output, uint256 amountIn, uint256 minOut) private returns (uint256 amountOut) {
