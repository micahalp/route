...
contract RouteFeeExecutor is ReentrancyGuard, Ownable2Step {
    ...
    event FeePaid(address indexed sender, address indexed recipient, address indexed token, uint256 grossAmountOut, uint256 feeAmount);
    ...
    struct Settlement { uint256 inputBalance; uint256 outputBalance; address treasury; }

    constructor(address route, address meta, address admin, address treasury) Ownable(admin) {
        if (route.code.length == 0 || meta.code.length == 0 || route == meta) revert InvalidSwap();
    ...
    receive() external payable {
        if (msg.sender != address(routeExecutor) && msg.sender != address(metaExecutor)) revert InvalidSwap();
    }

    function setFeeRecipient(address treasury) external onlyOwner { _setFeeRecipient(treasury); }
    function _setFeeRecipient(address treasury) private {
        if (treasury == address(0) || treasury == address(this) || treasury == address(routeExecutor) || treasury == address(metaExecutor)) revert InvalidFeeRecipient();
    ...
    function allowedAdapter(address adapter) external view returns (bool) { return routeExecutor.allowedAdapter(adapter); }
    function wrappedNative() external view returns (address) { return address(routeExecutor.wrappedNative()); }

    function feeFor(uint256 gross) public pure returns (uint256) { return Math.mulDiv(gross, FEE_BPS, 10_000); }
    ...
    function swap(address tokenIn, address tokenOut, uint256 amountIn, uint256 minOut, address recipient,
        uint256 deadline, RouteExecutor.Branch[] calldata branches) external payable nonReentrant returns (uint256 amountOut)
    {
        Settlement memory state = _start(tokenIn, tokenOut, amountIn, minOut, recipient, deadline, address(routeExecutor));
        routeExecutor.swap{value: msg.value}(tokenIn, tokenOut, amountIn, grossForNet(minOut), address(this), deadline, branches);
    ...
    function swap(address tokenIn, address tokenOut, uint256 amountIn, uint256 minOut, address recipient,
        uint256 deadline, bytes calldata data) external payable nonReentrant returns (uint256 amountOut)
    {
        Settlement memory state = _start(tokenIn, tokenOut, amountIn, minOut, recipient, deadline, address(metaExecutor));
        metaExecutor.swap{value: msg.value}(tokenIn, tokenOut, amountIn, grossForNet(minOut), address(this), deadline, data);
    ...
    function _start(address tokenIn, address tokenOut, uint256 amountIn, uint256 minOut, address recipient, uint256 deadline, address engine)
        private returns (Settlement memory state)
    {
        if (block.timestamp > deadline) revert DeadlineExpired();
        if (tokenIn == tokenOut || amountIn == 0 || minOut == 0 || recipient == address(0) || recipient == address(this)
            || recipient == address(routeExecutor) || recipient == address(metaExecutor)
        ...
        if (tokenIn != address(0)) {
            ...
            if (_balance(tokenIn) != state.inputBalance + amountIn) revert UnsupportedToken();
            IERC20(tokenIn).forceApprove(engine, amountIn);
        }
    }
    function _finish(Settlement memory state, address tokenIn, address tokenOut, uint256 minOut, address recipient, address engine)
        private returns (uint256 amountOut)
    {
        if (tokenIn != address(0)) IERC20(tokenIn).forceApprove(engine, 0);
        if (_balance(tokenIn) != state.inputBalance) revert UnsupportedToken();
        uint256 gross = _balance(tokenOut) - state.outputBalance;
        uint256 fee = feeFor(gross);
        amountOut = gross - fee;
        if (amountOut < minOut) revert SlippageExceeded();
        if (fee != 0) _pay(tokenOut, state.treasury, fee);
        _pay(tokenOut, recipient, amountOut);
        if (_balance(tokenOut) != state.outputBalance) revert UnsupportedToken();
    ...
    function _pay(address token, address recipient, uint256 amount) private {
        if (token == address(0)) {
            ...
            if (IERC20(token).balanceOf(recipient) != beforeRecipient + amount) revert UnsupportedToken();
        }
    }
}
