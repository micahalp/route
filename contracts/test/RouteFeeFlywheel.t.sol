...
contract FlyFactory {
    ...
    function transferCreatorFeeRecipient(address, address r) external {
        recipient = r;
    }
}
    ...
    function swap(
        ...
        if (reenter) {
            (bool ok,) = msg.sender
                .call(
                    abi.encodeCall(
                        RouteFeeFlywheel.execute, (0, 1 ether, 1, block.timestamp, branches)
                    )
                );
...
contract RouteFeeFlywheelTest {
    ...
    function setUp() public {
        ...
        safe = new FlySafe();
        token = new FlyToken();
        escrow = new FlyEscrow();
        factory = new FlyFactory();
        executor = new FlyExecutor();
        wheel = new RouteFeeFlywheel(
            address(safe),
            OPERATOR,
            address(token),
            address(escrow),
    ...
    function run(uint256 seq, uint256 amount, uint256 min) private {
        ...
        vm.prank(OPERATOR);
        wheel.execute(seq, amount, min, block.timestamp, branches);
    }

    function testStartsPausedAndDeployerCannotActivate() public {
        require(wheel.paused());
        vm.expectRevert(RouteFeeFlywheel.Unauthorized.selector);
        wheel.setPolicy(terms());
    }
    ...
    function testFuzzSplit(uint96 raw) public {
        ...
        activate();
        escrow.credit(address(wheel), amount);
    ...
    function testLowFloorRejectedBeforeClaim() public {
        activate();
        escrow.credit(address(wheel), 1 ether);
        vm.expectRevert(RouteFeeFlywheel.SlippageExceeded.selector);
        run(0, 1 ether, 1);
    ...
    function testExtraEscrowCreditsCannotChangeBatch() public {
        activate();
        escrow.credit(address(wheel), 2 ether);
        run(0, 1 ether, 65 ether);
    ...
    function testDuplicateAndEarlyExecutionRejected() public {
        ...
        run(0, 1 ether, 65 ether);
        escrow.credit(address(wheel), 1 ether);
        vm.expectRevert(RouteFeeFlywheel.InvalidBatch.selector);
        run(0, 1 ether, 65 ether);
        vm.expectRevert(RouteFeeFlywheel.InvalidBatch.selector);
        run(1, 1 ether, 65 ether);
    ...
    function testExpiry() public {
        ...
        vm.expectRevert(RouteFeeFlywheel.Inactive.selector);
        run(0, 1 ether, 65 ether);
    }

    function testBudgetAndBatchLimit() public {
        RouteFeeFlywheel.Policy memory p = terms();
        p.remainingClaimsBudget = 1 ether;
        vm.prank(address(safe));
        wheel.setPolicy(p);
        vm.expectRevert(RouteFeeFlywheel.InvalidBatch.selector);
        run(0, 2 ether, 130 ether);
        vm.expectRevert(RouteFeeFlywheel.InvalidBatch.selector);
        run(0, 11 ether, 715 ether);
    }

    function testOperatorCannotRecoverOrRedirect() public {
        vm.expectRevert(RouteFeeFlywheel.Unauthorized.selector);
        vm.prank(OPERATOR);
        wheel.recover(address(0), true);
        vm.expectRevert(RouteFeeFlywheel.Unauthorized.selector);
        vm.prank(OPERATOR);
        wheel.handoverFeesToTreasury();
    }
    ...
    function testRecoveryRequiresPauseAndDonationsDoNotChangeSplit() public {
        activate();
        vm.deal(address(wheel), 7 ether);
        escrow.credit(address(wheel), 1 ether);
        run(0, 1 ether, 65 ether);
        require(address(wheel).balance == 7 ether);
        vm.expectRevert(RouteFeeFlywheel.Inactive.selector);
        vm.prank(address(safe));
        wheel.recover(address(0), false);
        vm.prank(address(safe));
        wheel.pause();
        vm.prank(address(safe));
        wheel.recover(address(0), false);
    ...
    function testReentryFails() public {
        activate();
        escrow.credit(address(wheel), 1 ether);
        executor.configure(100 ether, address(0), true);
        run(0, 1 ether, 65 ether);
    }
}
