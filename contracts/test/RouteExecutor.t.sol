...
contract RouteExecutorTest {
    ...
    function leg(address adapter, address output) internal pure returns (RouteExecutor.Leg memory) {
        return RouteExecutor.Leg(adapter, output, 0, 0, address(0), false);
    }
    ...
    function direct(uint256 amount) internal view returns (RouteExecutor.Branch[] memory p) {
        ...
        p[0].legs[0] = leg(address(v1), address(b));
    }

    function testSplitAndClearApprovals() public {
        RouteExecutor.Branch[] memory p = new RouteExecutor.Branch[](2);
        p[0] = direct(4 ether)[0];
        p[1] = direct(6 ether)[0];
        p[1].legs[0].adapter = address(v2);
        vm.prank(user);
        uint256 out =
            router.swap(address(a), address(b), 10 ether, 26 ether, user, block.timestamp, p);
        require(
            out == 26 ether && a.allowance(address(router), address(v1)) == 0
                && a.allowance(address(router), address(v2)) == 0
        );
    }
    ...
    function testThreeHopsPreserveDonations() public {
        ...
        p[0].legs[0] = leg(address(v1), address(c));
        ...
        vm.prank(user);
        require(
            router.swap(address(a), address(b), 1 ether, 8 ether, user, block.timestamp, p)
                == 8 ether
        );
        require(
            a.balanceOf(address(router)) == 7 && b.balanceOf(address(router)) == 8
                && c.balanceOf(address(router)) == 9 && d.balanceOf(address(router)) == 10
        );
    }

    function testBadAllocationRejected() public {
        vm.prank(user);
        vm.expectRevert(RouteExecutor.InvalidAmount.selector);
        router.swap(address(a), address(b), 2 ether, 1, user, block.timestamp, direct(1 ether));
    }

    function testUnknownAdapterRejected() public {
        RouteExecutor.Branch[] memory p = direct(1);
        p[0].legs[0].adapter = address(dex1);
        vm.prank(user);
        vm.expectRevert(RouteExecutor.InvalidRoute.selector);
        router.swap(address(a), address(b), 1, 1, user, block.timestamp, p);
    }

    function testWrongOutputRejected() public {
        RouteExecutor.Branch[] memory p = direct(1);
        p[0].legs[0].tokenOut = address(c);
        vm.prank(user);
        vm.expectRevert(RouteExecutor.InvalidRoute.selector);
        router.swap(address(a), address(b), 1, 1, user, block.timestamp, p);
    }
    ...
    function testZeroMinimumRejected() public {
        vm.prank(user);
        vm.expectRevert(RouteExecutor.InvalidAmount.selector);
        router.swap(address(a), address(b), 1, 0, user, block.timestamp, direct(1));
    }

    function testExpiredRejected() public {
        vm.warp(100);
        vm.prank(user);
        vm.expectRevert(RouteExecutor.DeadlineExpired.selector);
        router.swap(address(a), address(b), 1, 1, user, 99, direct(1));
    }

    function testFinalSlippageRevertsAll() public {
        uint256 beforeBalance = a.balanceOf(user);
        vm.prank(user);
        vm.expectRevert(RouteExecutor.SlippageExceeded.selector);
        router.swap(
            address(a), address(b), 1 ether, 3 ether, user, block.timestamp, direct(1 ether)
        );
    ...
    function testNativeInput() public {
        ...
        require(
            router.swap{value: 1 ether}(
                address(0), address(b), 1 ether, 2 ether, user, block.timestamp, direct(1 ether)
            ) == 2 ether
        );
    }
    ...
    function testNativeOutput() public {
        ...
        vm.prank(user);
        router.swap(address(b), address(0), 1 ether, 1 ether, user, block.timestamp, p);
        require(user.balance == beforeBalance + 1 ether);
    }
    ...
    function testTaxedInputRejected() public {
        ...
        router.swap(address(tax), address(b), 1 ether, 1, user, block.timestamp, direct(1 ether));
    }
    ...
    function testCycleRejected() public {
        ...
        p[0].legs[0] = leg(address(v1), address(c));
        p[0].legs[1] = leg(address(v2), address(a));
        vm.prank(user);
        vm.expectRevert(RouteExecutor.InvalidRoute.selector);
        router.swap(address(a), address(b), 1, 1, user, block.timestamp, p);
    }

    function testNonV4ParametersRejected() public {
        RouteExecutor.Branch[] memory p = direct(1);
    ...
    function testFuzzSplitExactInput(uint96 input, uint8 fraction) public {
        ...
        require(
            beforeBalance - a.balanceOf(user) == amount && out == left * 2 + (amount - left) * 3
        );
    }
}
