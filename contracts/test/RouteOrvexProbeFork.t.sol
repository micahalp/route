...
contract OrvexForceDonation {
    constructor(address payable target) payable {
        selfdestruct(target);
    }
}

contract OrvexRejectRecipient {
...
contract OrvexReentryRecipient {
    ...
    constructor(RouteOrvexProbe target) {
        probe = target;
    }
...
contract RouteOrvexProbeForkTest {
    ...
    function _swap(address input, address output, uint256 amount, address to) private {
        if (input != address(0)) {
            IERC20(input).approve(address(probe), amount);
        }
        uint256 beforeInput = _balance(input, address(this));
        uint256 beforeOutput = _balance(output, to);
        uint256 out = probe.swap{value: input == address(0) ? amount : 0}(
            input, output, amount, 1, to, block.timestamp
        );
        require(out > 0 && _balance(input, address(this)) + amount == beforeInput);
        require(_balance(output, to) == beforeOutput + out);
        if (input != address(0)) {
            require(IERC20(input).allowance(address(this), address(probe)) == 0);
        }
        _donations();
    }

    function testNativeInputExactBalancesAndDonations() public {
        _swap(address(0), usdg, 0.01 ether, recipient);
    }

    function testWrappedInputExactBalancesAndDonations() public {
        _swap(weth, usdg, 0.01 ether, recipient);
    }

    function testNativeOutputExactBalancesAndDonations() public {
        _swap(usdg, address(0), 100e6, recipient);
    }

    function testWrappedOutputExactBalancesAndDonations() public {
        _swap(usdg, weth, 100e6, recipient);
    }

    function testCallerRecipientBothDirections() public {
        _swap(address(0), usdg, 0.01 ether, address(this));
        _swap(usdg, address(0), 100e6, address(this));
    }
    ...
    function testImpossibleMinimumRollsBack() public {
        ...
        require(
            IERC20(usdg).balanceOf(address(this)) == beforeInput
                && recipient.balance == beforeOutput
        );
        require(IERC20(usdg).allowance(address(this), address(probe)) == 100e6);
        _donations();
    ...
    function testExpiredDeadline() public {
        ...
        probe.swap{value: 0.01 ether}(
            address(0), usdg, 0.01 ether, 1, recipient, block.timestamp - 1
        );
        _donations();
    }

    function testUnsolicitedAndVaultSpoofCallbacks() public {
        vm.expectRevert(RouteOrvexProbe.CallbackRejected.selector);
        probe.lockAcquired("");
        vm.prank(probe.VAULT());
        vm.expectRevert(RouteOrvexProbe.CallbackRejected.selector);
        probe.lockAcquired("");
        _donations();
    }
    ...
    function testRejectingNativeRecipientRollsBack() public {
        ...
        require(IERC20(usdg).balanceOf(address(this)) == beforeInput);
        _donations();
    }
    ...
    function testForwardingNativeRecipientReceivesExactValue() public {
        ...
        require(
            IERC20(usdg).balanceOf(address(this)) + 100e6 == beforeInput
                && recipient.balance == beforeOutput + out
        );
        require(target.balance == 0);
        _donations();
    }
    ...
    function testNativeRecipientCannotReenterSwapOrCallback() public {
        ...
        _swap(usdg, address(0), 100e6, address(target));
