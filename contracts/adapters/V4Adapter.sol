...
interface IV4Wrapped is IERC20 {
...
contract V4Adapter is ReentrancyGuard {
    ...
    constructor(address manager, address wrapped) {
        if (manager.code.length == 0 || wrapped.code.length == 0) {
            revert InvalidSwap();
        }
        poolManager = IRoutePoolManager(manager);
        wrappedNative = IV4Wrapped(wrapped);
    }

    receive() external payable {
        if (msg.sender != address(poolManager) && msg.sender != address(wrappedNative)) {
    ...
    function swap(
        ...
        if (
            tokenIn == tokenOut || tokenIn == address(0) || tokenOut == address(0) || amountIn == 0
                || amountIn > uint256(uint128(type(int128).max)) || hooks != address(0)
                || fee >= 1_000_000 || tickSpacing <= 0
        ) {
            revert InvalidSwap();
        }
        ...
        if (nativePool && input != address(0) && output != address(0)) {
            revert InvalidSwap();
        }
        ...
        if (IERC20(tokenIn).balanceOf(address(this)) != beforeInput + amountIn) {
            revert UnsupportedToken();
        }
        if (input == address(0)) {
            wrappedNative.withdraw(amountIn);
        }
        IRoutePoolManager.PoolKey memory key = IRoutePoolManager.PoolKey(
        ...
        if (output == address(0)) {
            wrappedNative.deposit{value: amountOut}();
        }
        if (
            address(this).balance != beforeNative
                || IERC20(tokenIn).balanceOf(address(this)) != beforeInput
                || IERC20(tokenOut).balanceOf(address(this)) != beforeOutput + amountOut
        ) {
            revert UnsupportedToken();
        }
        uint256 beforeRecipient = IERC20(tokenOut).balanceOf(msg.sender);
        IERC20(tokenOut).safeTransfer(msg.sender, amountOut);
        if (IERC20(tokenOut).balanceOf(msg.sender) != beforeRecipient + amountOut) {
            revert UnsupportedToken();
        }
    }
    ...
    function unlockCallback(bytes calldata data) external returns (bytes memory) {
        ...
        pending = bytes32(0);
        (IRoutePoolManager.PoolKey memory key, address input, address output, uint256 amountIn) =
            abi.decode(data, (IRoutePoolManager.PoolKey, address, address, uint256));
        ...
        if (input == address(0)) {
            if (poolManager.settle{value: amountIn}() != amountIn) {
                revert UnsupportedToken();
            }
        } else {
