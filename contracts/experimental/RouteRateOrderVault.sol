...
contract RouteRateOrderVault is ReentrancyGuard {
    ...
    error InvalidOrder(); error NotOwner(); error NotEligible(); error UnauthorizedKeeper();
    error TransferMismatch(); error MinimumNotMet();
    ...
    constructor(address swapAdapter) {
        if(swapAdapter.code.length==0)revert InvalidOrder();
    ...
    function create(Terms calldata t) external nonReentrant returns(uint256 id) {
        if(t.market==t.settlement || t.market.code.length==0 || t.settlement.code.length==0 ||
            ...
            t.expiresAt<=block.timestamp || t.expiresAt>block.timestamp+365 days)revert InvalidOrder();
        ...
        if((buy ? t.entryMinimum==0 : t.entryMinimum!=0) ||
            (tp ? t.takeProfitRate==0 : t.takeProfitRate!=0) ||
            (sl ? t.keeper==address(0) || t.stopTriggerRate==0 || t.stopMinimumRate==0 ||
                t.stopMinimumRate>t.stopTriggerRate :
                t.keeper!=address(0) || t.stopTriggerRate!=0 || t.stopMinimumRate!=0))revert InvalidOrder();
        if(t.kind==Kind.Bracket) {
            uint256 entryRate=Math.mulDiv(t.amount,1e18,t.entryMinimum,Math.Rounding.Ceil);
            if(t.stopTriggerRate>=entryRate || t.takeProfitRate<=entryRate)revert InvalidOrder();
        }
        ...
        id=++nextId;orders[id]=Order(msg.sender,State.Pending,t.amount,t);emit Created(id,msg.sender);
    }
    ...
    function execution(uint256 id,Leg leg) public view returns(address input,address output,uint256 amount,uint256 minimum) {
        ...
        if(leg==Leg.TakeProfit) {
            if(t.kind!=Kind.TakeProfit && t.kind!=Kind.Bracket)revert NotEligible();
    ...
    function _deliver(address token,address owner,uint256 amount) private {
        ...
        if(IERC20(token).balanceOf(address(this))!=beforeVault-amount ||
            IERC20(token).balanceOf(owner)!=beforeOwner+amount)revert TransferMismatch();
    }
}
