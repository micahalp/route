...
interface ICLFactory { function getPool(address, address, uint24) external view returns (address); }
interface ICLRouter {
...
contract CLAdapter is BaseAdapter {
    ...
    function quote(address, address, uint256) external pure returns (uint256, uint24) { return (0, 0); }
    function _execute(address input, address output, uint256 amount, uint256 minimum, uint24 fee) internal override {
        if (fee >= 1_000_000 || factory.getPool(input, output, fee) == address(0)) revert InvalidSwap();
        ICLRouter(dexRouter).exactInputSingle(ICLRouter.Params(input, output, fee, address(this), block.timestamp, amount, minimum, 0));
    }
}
