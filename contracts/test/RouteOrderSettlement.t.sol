...
contract RouteOrderSettlementTest {
    ...
    RouteLeanMetaExecutor executor;
    ...
    function setUp() public {
        ...
        feed.set(10 ether, 1000);
        vault = new RouteOrderVault(address(feed), address(executor), 60);
        cash.mint(address(this), 100 ether);
        market.mint(address(upstream), 100 ether);
        cash.mint(address(upstream), 100 ether);
        cash.approve(address(vault), 100 ether);
    }

    function create() internal returns (uint256) {
        return vault.create(
            RouteOrderVault.Terms({
                kind: RouteOrderVault.Kind.Bracket,
                market: address(market),
                settlement: address(cash),
                amount: 10 ether,
                entryPrice: 10 ether,
                takeProfitPrice: 12 ether,
                stopPrice: 8 ether,
                entryMinimum: 15 ether,
                takeProfitRate: 2 ether,
                stopRate: 0.5 ether,
                expiresAt: 2000
            })
        );
    ...
    function enter(uint256 id, uint256 amount, uint256 received, address recipient) internal {
        vault.execute(
            id,
            RouteOrderVault.Leg.Entry,
            payload(address(cash), address(market), amount, received, recipient)
        );
    }
    ...
    function testBracketUsesActualOutputAndClearsBothAllowanceLayers() public {
        ...
        vault.execute(
            id,
            RouteOrderVault.Leg.TakeProfit,
    ...
    function testRedirectedOutputRollsBackWholeOrder() public {
        ...
        require(vault.getOrder(id).state == RouteOrderVault.State.Pending);
        assertClean();
    }
    ...
    function testDonationsCannotSatisfyMinimumOrBeSpent() public {
        uint256 id = create();
        market.mint(address(vault), 50 ether);
        cash.mint(address(executor), 3 ether);
        market.mint(address(executor), 4 ether);
        vm.expectRevert(RouteLeanMetaExecutor.SlippageExceeded.selector);
        enter(id, 10 ether, 14 ether, address(vault));
        enter(id, 10 ether, 20 ether, address(vault));
        require(vault.getOrder(id).held == 20 ether && market.balanceOf(address(vault)) == 70 ether);
        require(
            cash.balanceOf(address(executor)) == 3 ether
                && market.balanceOf(address(executor)) == 4 ether
        );
        vault.cancel(id);
        require(market.balanceOf(address(this)) == 20 ether);
        require(market.balanceOf(address(vault)) == 50 ether);
        assertClean();
    }
    ...
    function testRejectedStopKeepsPositionAvailableForCancellation() public {
        uint256 id = create();
        enter(id, 10 ether, 20 ether, address(vault));
        feed.set(7 ether, 1000);
        vm.expectRevert(RouteLeanMetaExecutor.SlippageExceeded.selector);
        vault.execute(
            id,
            RouteOrderVault.Leg.StopLoss,
