...
contract RouteFeeFlywheel is ReentrancyGuardTransient {
    ...
    uint256 public constant MAX_POLICY_DURATION = 1 days;
    ...
    constructor(
        ...
        if (
            safe_.code.length == 0 || operator_ == address(0) || token_.code.length == 0
                || escrow_.code.length == 0 || factory_.code.length == 0
                || executor_.code.length == 0 || safe_ == executor_ || safe_ == escrow_
        ) {
    ...
    function pause() external onlyTreasury nonReentrant {
        paused = true;
        emit Paused();
    }
    ...
    function execute(
        ...
        uint256 minOut,
        uint256 deadline,
        IFlywheelExecutor.Branch[] calldata branches
    ) external nonReentrant returns (uint256 received) {
        if (msg.sender != operator) {
            revert Unauthorized();
        }
        Policy memory p = policy;
        if (paused || block.chainid != chainId || block.timestamp > p.expiresAt) {
            revert Inactive();
        }
        if (
            expectedSequence != sequence || deadline < block.timestamp || deadline > p.expiresAt
                || expectedClaim < p.minClaim || expectedClaim > p.maxClaim
                || expectedClaim > p.remainingClaimsBudget
                || (lastRun != 0 && block.timestamp < lastRun + p.minInterval)
        ) {
            revert InvalidBatch();
        }
        ...
        if (address(this).balance != beforeEth + expectedClaim) {
            revert InvalidBatch();
        }
        ...
        if (address(this).balance != beforeEth) {
            revert InvalidBatch();
        }
    ...
    function recover(address asset, bool claimFirst) external onlyTreasury nonReentrant {
        ...
        if (asset == address(0)) {
            if (claimFirst) {
                IFlywheelEscrow(escrow).claim();
            }
            amount = address(this).balance;
            _send(amount);
        } else {
            if (claimFirst) {
                IFlywheelEscrow(escrow).claimToken(asset);
            }
