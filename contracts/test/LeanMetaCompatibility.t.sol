...
contract ForwardingNativeRecipient {
    ...
    receive() external payable {
        (bool sent,) = destination.call{value: msg.value}(""); require(sent);
    }
}
contract LeanMetaCompatibilityTest {
    ...
    function setUp() public {
        token = new Token("Input"); upstream = new MockUpstream();
        ...
        token.mint(user, 10 ether); vm.deal(address(upstream), 10 ether);
        vm.prank(user); token.approve(address(lean), 1 ether);
        vm.prank(user); token.approve(address(current), 1 ether);
    }
    function testLeanNativeForwardingRejectionIsAtomic() public {
        bytes memory data = abi.encodeCall(MockUpstream.swap, (address(token), address(0), 1 ether, 2 ether, address(recipient)));
        vm.prank(user); vm.expectRevert(RouteLeanMetaExecutor.SlippageExceeded.selector);
        lean.swap(address(token), address(0), 1 ether, 2 ether, address(recipient), block.timestamp, data);
    ...
    function testExistingNativePathPreservesForwardingRecipientAndDonations() public {
        token.mint(address(current), 7); vm.deal(address(current), 8);
        bytes memory data = abi.encodeCall(MockUpstream.swap, (address(token), address(0), 1 ether, 2 ether, address(current)));
        vm.prank(user);
        require(current.swap(address(token), address(0), 1 ether, 2 ether, address(recipient), block.timestamp, data) == 2 ether);
        ...
        require(token.allowance(user, address(current)) == 0 && token.allowance(address(current), address(upstream)) == 0);
    }
    function testTokenDeliveryToContractRecipientRemainsSupported() public {
        Token output = new Token("Output"); output.mint(address(upstream), 2 ether);
        bytes memory data = abi.encodeCall(MockUpstream.swap, (address(token), address(output), 1 ether, 2 ether, address(recipient)));
        vm.prank(user);
        require(lean.swap(address(token), address(output), 1 ether, 2 ether, address(recipient), block.timestamp, data) == 2 ether);
        require(output.balanceOf(address(recipient)) == 2 ether);
        require(token.balanceOf(user) == 9 ether && token.allowance(address(lean), address(upstream)) == 0);
    }
}
